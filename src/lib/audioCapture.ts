// Source Kit 0.1.0 — bridge to the native AudioCapture module
...
export interface AudioStopResult {
  ...
  /**
   * ENF anchor + integrity summary for the committed raw master (0.20.1),
   * the same shape the video tee's stop payload carries. Present only when
   * rawPcmState is 'recorded'; ABSENT (undefined) on pre-0.20.1 native
   * builds — callers treat absence as "no anchor sealed" (the details
   * card's parser then has nothing to arbitrate with and says so).
   */
  rawPcmInfo?: {
