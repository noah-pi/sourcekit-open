...
export interface SensorContext {
  ...
  depth?:
    ...
    | null;
  /**
   * Power-grid (ENF) anchor for the raw-audio master (native
   * Present only on video records whose PCM sink committed. The
...
export interface AttestationRecord {
  ...
  /** Present on de-identified re-signs: the signer stripped identity /
      location / sensor claims before sharing. Media hash unchanged; removed
      context is gone from the copy, not hidden. */
  deidentified?: {
    at: string;
    fields: string[];
    /**+: signed with a fresh one-time key, breaking fingerprint linkage
  ...
  /** Present when the capture signed with a per-assignment key (protection) instead of the device key. The label is self-asserted; what
  ...
  /** Stereo-capture artifact section:
