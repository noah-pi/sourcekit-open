...
export interface Settings {
  ...
  /**
   * 0.22.0: the capture-time Face check is REMOVED — its LA prompt
   * contended with the live multicam session and killed dual camera on
   * every capture in the field. The sealed telemetry field
   * (`captureIntegrity.biometricGatePassed`) stays in the record format:
   * null means "not requested", which is now always the case at capture.
   * The seal-time biometric hold (biometricSigning) and app-lock Face ID
   * (biometricsEnabled) are unaffected — neither touches the camera path.
   * A stale `faceCheckEnabled` key in an existing settings.json is ignored.
   */
  /**
  ...
  /**
   * 0.22.0: the SDK signing A/B is over (zero quarantines in the field
   * trial) — the c2pa-swift SDK is the default engine for photo and video
   * with the hand-rolled fallback intact, the toggle is gone, and the
   * runtime gate (src/lib/sdkSigningGate.ts) is a constant. A stale
   * `sdkSigningExperiment` key in an existing settings.json is ignored.
   */
  /**
