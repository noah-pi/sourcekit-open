...
export interface Settings {
  ...
  /**
   * Byline inclusion (the camera "Name" HUD toggle): when on AND
  ...
  /**
   * Audio transcript embedding (toggle board): when on, voice notes
  ...
  /**
   * Assignment mode (source protection): when non-empty, captures sign
  ...
  videoTorch: boolean;
  /**
   * Appearance (Track E): 'device' follows the iPhone's dark-mode
   ...
   * and older stores (which then migrate on next load).
   */
  ...
  loadSettings: async () => {
    try {
      ...
      if (info.exists) {
        ...
        // older migration: includeIdentity (boolean) → identityMode.
        ...
        // Drop the experimental captureKit toggle key from stored settings.
        delete stored.captureKitEnabled;
        // migration — one-time, guarded:
        ...
        // user's store was written (captureEvidence.altView). Rebuild
