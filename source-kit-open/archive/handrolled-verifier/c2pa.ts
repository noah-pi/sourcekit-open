...
export async function buildC2paSegment(p: C2paManifestParams, insertOffset: number): Promise<Uint8Array> {
  ...
  const SLACK = 64; // absorbs TSA token-size variance without a re-sign
  ...
  for (let i = 0; i < 12; i++) {
    ...
    if (converged) {
      ...
      // The claim is signed; the exclusion length is sealed inside it. A
      // token that drifted beyond the slack leaves no honest repair.
      throw new Error('C2PA segment did not converge after signing (token size drift)');
    }
...
export async function buildC2paStorePng(p: C2paManifestParams, insertOffset: number): Promise<Uint8Array> {
  ...
  for (let i = 0; i < 12; i++) {
    ...
    if (converged) {
      ...
      throw new Error('C2PA PNG store did not converge after signing (token size drift)');
    }
